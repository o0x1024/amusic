...
export const HIT_CREATION_STAGES: HitCreationStageConfig[] = [
  ...
  { value: '90秒微型歌曲', label: '90 秒微型歌曲', durationLabel: '90s', description: '兼顾短视频传播与汽水完整收听', requiresLoop: false, structure: '0–3s 入口 → 3–20s 短主歌 → 20–28s 预副歌 → 28–50s 核心 Hook → 50–68s 变体 → 68–75s 情绪推进 → 75–90s 最终 Hook' },
...
export function getCreationStageConfig(stage?: HitCreationStage): HitCreationStageConfig {
  const normalizedStage = normalizeCreationStage(stage)
  return HIT_CREATION_STAGES.find(item => item.value === normalizedStage) || HIT_CREATION_STAGES[0]
}

export function normalizeCreationStage(stage?: HitCreationStage): HitCreationStage {
  return stage === '60秒微型歌曲' ? '90秒微型歌曲' : (stage || 'Hook探索')
}
