...
const EMPTY_STATE: HitIntelligenceState = { tasteReferences: [], trendSamples: [], publishMetrics: [], recentIdeas: [] }
...
export function loadHitIntelligence(): HitIntelligenceState {
  ...
  try {
    ...
    return {
      ...
      recentIdeas: Array.isArray(data.recentIdeas) ? data.recentIdeas : []
    }
  } catch (error) {
