...
6. 除上述 JSON 字段外禁止输出任何额外字段，尤其禁止 title、lyrics、Hook 或歌词 DNA
7. 不输出 Markdown、解释或候选正文`
  ...
  const generateCandidate = async (index: number, validationError = ''): Promise<HitLabResult> => {
    ...
    try {
      ...
      const raw = jsonFromText(response.content) as { variants?: unknown[] }
      if (!Array.isArray(raw.variants) || raw.variants.length !== 1) throw new ModelJsonError('variants 必须且只能包含 1 个候选')
      const missingFields = creationContractErrors(raw.variants[0], candidateId)
