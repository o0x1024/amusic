...
import { openingMechanismForCandidate } from '../shared/hit-opening'
import { hitLabOutputTokenBudget } from './hit-generation-budget'
import { creationContractErrors, evaluationContractErrors, hitLabCandidateId, mergeHitLabEvaluation } from './hit-lab-contract'
...
const SONG_RESULT_JSON_EXAMPLE = jsonExample({
  title: '歌曲标题',
  concept: '一句话概念描述',
  lyrics: '[Verse 1]\n第一句歌词\n第二句歌词\n\n[Chorus]\n副歌第一句\n副歌第二句',
  stylePrompt: 'English style prompt',
  arrangement: 'English arrangement prompt',
  vocalPrompt: 'English vocal prompt',
  fullPrompt: 'Genre: ...\nTempo: ...\nKey: ...\nMood: ...\nInstruments: ...\nVocal: ...\nProduction: ...\nStructure: ...\nDynamics: ...',
  sunoPrompt: 'English Suno prompt',
  udioPrompt: 'English Udio prompt',
  miaoxiangPrompt: 'English Miaoxiang prompt',
  lyricOnly: '[Verse 1]\n第一句歌词\n第二句歌词\n\n[Chorus]\n副歌第一句\n副歌第二句',
  negativePrompt: 'English negative prompt',
  qualityChecks: ['质量检查一', '质量检查二', '质量检查三', '质量检查四', '质量检查五'],
  revisionSuggestions: ['修改建议一', '修改建议二', '修改建议三'],
  variants: [{
    title: '候选方向一',
    direction: '第一个候选方向说明',
    hookLine: '第一个候选核心句',
    stylePrompt: 'First English variant style prompt',
    revisionFocus: '第一个修改重点'
  }, {
    title: '候选方向二',
    direction: '第二个候选方向说明',
    hookLine: '第二个候选核心句',
    stylePrompt: 'Second English variant style prompt',
    revisionFocus: '第二个修改重点'
  }, {
    title: '候选方向三',
    direction: '第三个候选方向说明',
    hookLine: '第三个候选核心句',
    stylePrompt: 'Third English variant style prompt',
    revisionFocus: '第三个修改重点'
  }]
})

const LYRICS_DRAFT_JSON_EXAMPLE = jsonExample({
  title: '歌名',
  concept: '一句话概念描述',
  lyrics: '[Verse 1]\n第一句歌词\n第二句歌词\n\n[Chorus]\n副歌第一句\n副歌第二句',
  hookLine: '最适合反复的一句歌词',
  lyricAnalysis: '歌词视角、句式密度、押韵与意象分析',
  emotionCurve: '克制主歌 -> 预副歌推升 -> 副歌释放',
  shortVideoMoment: '适合传播的段落及原因',
  revisionSuggestions: ['修改建议一', '修改建议二', '修改建议三']
})

const HIT_LAB_CREATION_JSON_EXAMPLE = jsonExample({
  summary: '本轮实验策略总结',
  variants: [{
    candidateId: 'candidate-1',
    title: '歌名',
    positioning: '听众、情绪和传播角度',
    targetPlatform: '双平台',
    hookLine: '最抓耳的一句歌词',
    openingMode: '当前候选指定的开场机制',
    firstThreeSeconds: '前 3 秒听感摘要',
    introHook: '前奏唯一抓耳机制',
    introTimeline: '0-1s：...；1-3s：...；3-8s：...',
    hookEntrySeconds: 3,
    signatureSound: '一秒内可识别的标志声音',
    loopTransition: '片段尾部接回开头的方法',
    introRisks: ['前奏风险一', '前奏风险二'],
    targetDurationSeconds: 30,
    structurePlan: '0-3s：...；3-8s：...；8-23s：...；23-30s：...',
    completenessTest: '目标时长内构成完整作品的原因',
    chorusSnippet: '副歌第一行\n副歌第二行\n副歌第三行\n副歌第四行',
    lyrics: '[Hook]\n第一句歌词\n第二句歌词\n第三句歌词\n第四句歌词',
    centralImage: '唯一中心意象',
    emotionalParadox: '一句话情绪矛盾',
    narrativeMode: '实际叙事引擎',
    hookSpeechAct: 'Hook 言语动作',
    specificDetails: ['歌词中的具体细节一', '歌词中的具体细节二'],
    perspectiveDistance: '人称与听众距离',
    repeatPattern: '重复与变体方式',
    storyProgression: '开场、推进、转折和落点',
    clicheRisks: ['陈词风险一', '陈词风险二'],
    derivativeSimilarityRisks: ['相似风险一'],
    stylePrompt: '流派、BPM、人声、核心乐器、制作质感',
...
function jsonFromText(text: string): unknown {
  ...
  try {
    return parseModelJson(trimmed)
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error)
    throw new ModelJsonError(detail)
  }
}
...
function normalizeHitLabVariant(value: unknown, index: number): HitLabVariant {
  ...
  const targetDurationSeconds = boundedSeconds(data.targetDurationSeconds, 300)
  return {
    ...
    openingMode: data.openingMode?.trim() || '',
    ...
    hookEntrySeconds: boundedSeconds(data.hookEntrySeconds, targetDurationSeconds || 300),
...
必须严格匹配下面的 JSON 结构和字段类型；示例值仅用于说明格式，生成时必须替换为真实内容：
${SONG_RESULT_JSON_EXAMPLE}
...
export async function generateHitLab(request: HitLabRequest): Promise<HitLabResult> {
  ...
  const sourceMaterial = usesExistingLyrics
    ...
    : `## 核心创意\n\n${request.idea}\n\n## 歌词路线\n\n${request.lyricRouteContext || '用户未指定歌词路线，请根据核心创意自主设计最合适的叙事方式和 Hook 机制'}`
...
- **variants**: 候选数组，数量必须等于用户要求的版本数。每个候选包含：
  ...
  - **specificDetails**: JSON 字符串数组，格式必须是 ["细节一","细节二"]；至少两个不可互换、且实际写进 lyrics 的具体细节。禁止返回编号文本、分号拼接字符串或对象
  - **perspectiveDistance**: 人称与听众距离
  - **repeatPattern**: 重复与变体方式
  - **storyProgression**: 开场、推进、转折和落点
...
必须严格匹配下面的 JSON 结构和字段类型；示例值仅用于说明格式，生成时必须替换为真实内容。字符串内的换行必须编码为反斜杠加 n，禁止在 JSON 字符串中直接插入裸换行：
${HIT_LAB_CREATION_JSON_EXAMPLE}
...
- 开场机制：${usesExistingLyrics ? '沿用原文开场，不为套模板改写' : request.openingMode || '自动选择'}
...
歌词 DNA 硬性要求：遵守“2—1—1”标准——至少 2 个不可互换的具体细节、只有 1 个中心意象、只有 1 个核心情绪判断。specificDetails 必须是 JSON 数组，例如 ["蓝漆掉漆的收银台","只剩半块的橡皮"]，数组每一项必须是歌词正文中真实出现的不同细节；禁止写成 "1. 细节一；2. 细节二"。主歌提供证据，Hook 给出可借用的判断；第二次重复必须推进含义。主动识别陈词风险和与现有作品过近的结构风险。

开场机制规则：开场是“第一句如何进入”，不等同于整首歌词的叙事路线。必须执行当前候选指定的开场机制，并把该指定值逐字写入 openingMode：
- “核心句直入”：第一句直接给 Hook 或核心判断，不用场景铺垫
- “动作现场”：从会改变局面的具体动作切入，动作必须推动人物处境，不能只是罗列环境
- “直接对话”：第一句是人物真正说出口的话，并立刻显露关系或冲突
- “中心意象”：让全歌唯一中心意象在第一句出现，并承担情绪功能
- “反常判断”：先给一个违反直觉但能由后文证明的判断，必须连接 emotionalParadox
- “声音拟声”：以可辨识的拟声、呼吸、口头声或环境声进入，并在一至两句内接上主题
- “结果先行”：先说出后果、决定或结局，再回推原因
- “原文开场（不得改写）”：仅用于已有歌词，保持原文第一句和原有次序

生活化不是默认开场。咖啡、便利店、手机没电、夜风、末班车、加班等高频都市道具，不能仅为制造“真实感”而出现；只有当细节会改变人物行动、核心矛盾或 Hook 含义时才可使用。具体细节必须是主题证据，不得把互不相关的日常名词排成氛围清单。完整歌曲的主歌也不必默认从日常场景起手。

叙事路线要求：${usesExistingLyrics
    ? '从原文诊断中识别，不擅自更换原文叙事。'
    : request.lyricRouteContext
      ? `严格执行用户已选路线，不得擅自换成更常见的爱情、失恋或都市孤独路线：\n${request.lyricRouteContext}`
      : '用户未指定歌词路线。根据核心创意自主选择一个清晰、具体的叙事引擎，并在所有候选中保持该引擎。'}
...
- hookEntrySeconds 必须与 introTimeline、structurePlan 和 fullPrompt 一致，且不得晚于 targetDurationSeconds；完整歌曲超过 30 秒可以接受，但应确保前 3 秒已有独立记忆点
...
- “只改开场机制”只改变 openingMode 与第一句的进入动作，锁定叙事引擎、中心意象、人物事实、情绪矛盾和核心 Hook 判断
...
当前阶段只负责创作事实，不要输出 lyricScores、douyinScore、qishuiScore、memorabilityScore、spreadPotential、shortVideoUseCases、riskNotes 或 iterationAdvice；这些字段由独立评价阶段产生。`
  ...
  const outputTokenBudget = hitLabOutputTokenBudget(getGenerationParams().maxTokens, 1, stageConfig.value)
  const evaluationTokenBudget = Math.max(getGenerationParams().maxTokens, 1800)

  const evaluationSystemPrompt = `你是爆款歌曲候选的独立评价器。你不创作、不改写歌词，只根据输入候选进行结构化评价。

只输出严格 JSON：
{"candidateId":"candidate-1","lyricScores":{"hookCompression":0,"specificity":0,"conversationalNaturalness":0,"narrativeProgression":0,"singability":0,"quoteDesire":0,"remixOpenness":0,"fullSongExpansion":0},"douyinScore":0,"qishuiScore":0,"memorabilityScore":0,"spreadPotential":"...","shortVideoUseCases":["..."],"riskNotes":["..."],"iterationAdvice":"..."}

要求：
1. 所有评分必须是 0-100 的有限数字，允许低分和 0 分
2. shortVideoUseCases 必须有 3-5 项，riskNotes 必须有 2-4 项
3. 评分必须基于候选实际文本，不得补写或修改候选
4. 明确区分抖音传播潜力、汽水完整收听价值和歌词自身质量
5. candidateId 必须原样返回，不得修改
6. 检查开场是否真正执行 openingMode；若只是堆砌咖啡、便利店、手机、夜风、末班车、加班等通用生活道具，或细节不推动行动、矛盾与 Hook，必须在 specificity、narrativeProgression 中降分并写入 riskNotes
7. 除上述 JSON 字段外禁止输出任何额外字段，尤其禁止 title、lyrics、Hook 或歌词 DNA
8. 不输出 Markdown、解释或候选正文`

  const generateEvaluation = async (candidateId: string, candidate: HitLabVariant, validationError = ''): Promise<HitLabVariant> => {
    const evaluationInput: Record<string, unknown> = { ...candidate }
    for (const field of ['lyricScores', 'douyinScore', 'qishuiScore', 'memorabilityScore', 'spreadPotential', 'shortVideoUseCases', 'riskNotes', 'iterationAdvice']) {
      delete evaluationInput[field]
    }
    const prompt = `## 候选 ID\n\n${candidateId}\n\n## 待评价候选\n\n${JSON.stringify(evaluationInput)}\n\n请原样返回 candidateId，并输出该候选的独立结构化评价。`
      + (validationError ? `\n\n上次评价缺失或无效字段：${validationError}。本次必须完整返回。` : '')
    const response = await chat(config, evaluationSystemPrompt, prompt, { maxTokens: evaluationTokenBudget })
    if (!response.success) throw new Error(response.error ?? 'AI 评价失败')

    try {
      if (isLengthLimit(response.finishReason)) throw new ModelJsonError(`模型评价输出达到长度上限（finish_reason=${response.finishReason}）`)
      const raw = jsonFromText(response.content) as Record<string, unknown>
      const missing = evaluationContractErrors(raw, candidateId)
      if (missing.length) throw new ModelJsonError(missing.join('、'))
      return mergeHitLabEvaluation(candidate, raw)
    } catch (error) {
      if (!(error instanceof ModelJsonError)) throw error
      logger.warn('爆款实验台独立评价不符合结构契约', { title: candidate.title, error: error.message })
      if (!validationError) return generateEvaluation(candidateId, candidate, error.message)
      throw new Error(`候选《${candidate.title}》连续两次未通过评价契约：${error.message}`)
    }
  }

  const generateCandidate = async (index: number, validationError = ''): Promise<HitLabResult> => {
    const candidateId = hitLabCandidateId(index)
    const requestedOpeningMode = request.mutationFocus === '只改开场机制' ? '自动选择' : request.openingMode
    const openingMechanism = usesExistingLyrics
      ? '原文开场（不得改写）'
      : openingMechanismForCandidate(requestedOpeningMode, index)
    const base = request.mutationFocus === '只改开场机制'
      ? '保持同一叙事引擎、人物事实、核心意象、情绪判断、Hook 和音乐定位，只测试第一句的进入方式'
      : candidateDirections[index % candidateDirections.length]
    const direction = `${index + 1}. ${base}${index >= candidateDirections.length ? '，且不得重复前四个候选的核心机制' : ''}\n开场机制：${openingMechanism}。openingMode 必须逐字返回“${openingMechanism}”。`
    const batchPrompt = prompt
      .replace(`- 候选数量：${versionCount}`, '- 候选数量：1')
      .replace(`请生成 ${versionCount} 个候选版本。`, '请只生成 1 个候选版本。')
      + `\n\n## 当前候选方向\n\n${direction}\n\n候选 ID：${candidateId}。这是总实验的第 ${index + 1} 个候选。variants 必须且只能包含这一个候选，并原样返回 candidateId。每个文本字段保持简洁，禁止复述要求、解释推理或输出其他候选。`
      + (validationError ? `\n\n## 上次输出违反结构契约\n\n错误：${validationError}。请重新生成完整候选并纠正字段类型。尤其注意 specificDetails 必须直接输出为 ["细节一","细节二"]，不得输出为字符串。` : '')

    const response = await chat(config, systemPrompt, batchPrompt, { maxTokens: outputTokenBudget })
    ...
    try {
      ...
      const raw = jsonFromText(response.content) as { summary?: unknown; variants?: unknown[] }
      if (typeof raw.summary !== 'string' || !raw.summary.trim()) throw new ModelJsonError('summary')
      if (!Array.isArray(raw.variants) || raw.variants.length !== 1) throw new ModelJsonError('variants 必须且只能包含 1 个候选')
      const missingFields = creationContractErrors(raw.variants[0], candidateId, openingMechanism)
      if (missingFields.length) throw new ModelJsonError(missingFields.join('、'))
      const batch = normalizeHitLabResult(raw)
      ...
      const evaluated = await generateEvaluation(candidateId, batch.variants[0])
      return { summary: batch.summary, variants: [evaluated] }
    } catch (error) {
      if (!(error instanceof ModelJsonError)) throw error
      logger.warn('爆款实验台候选响应不符合结构契约', {
        candidateIndex: index,
        finishReason: response.finishReason,
        error: error.message
      })
      if (!validationError) return generateCandidate(index, error.message)
      throw new Error(`候选 ${index + 1} 连续两次未通过结构契约：${error.message}`)
    }
  }

  const batchResults: HitLabResult[] = []
  for (let index = 0; index < versionCount; index += 2) {
    const candidateIndexes = Array.from({ length: Math.min(2, versionCount - index) }, (_, offset) => index + offset)
    batchResults.push(...await Promise.all(candidateIndexes.map(candidateIndex => generateCandidate(candidateIndex))))
  }
...
9. 如果用户已选择歌词路线，四张卡只能比较音乐机制，不得改变中心意象、叙事引擎、人称距离和 Hook 言语动作`
  ...
  const strategySource = request.sourceMode === 'lyrics'
    ...
    : request.lyricRouteContext
      ...
      : `核心创意：${request.idea}\n\n用户未指定歌词路线，音乐策略需要同时给出与各自音乐机制匹配的歌词叙事方向和 Hook 方式。`
...
请生成 4 张真正不同的策略卡。即使用户指定了主要爆点，也要让四张卡通过不同音乐机制实现该目标，而不是给出四个近义版本。`
  ...
  if (!hasSufficientStrategyDiversity(result.cards)) {
    result.summary = [result.summary, '提示：本轮音乐策略相似度较高，已保留全部原始方案供比较。'].filter(Boolean).join('\n')
    logger.warn('爆款实验台音乐策略相似度较高，保留结果供用户比较', {
