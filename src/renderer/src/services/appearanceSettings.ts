...
const STORAGE_KEY = 'amusic-appearance-v2'
const LEGACY_THEME_KEY = 'amusic-theme'
...
export function loadAppearanceSettings(): AppearanceSettings {
  try {
    ...
    if (raw) return normalize(JSON.parse(raw) as Partial<AppearanceSettings>)
  } catch {
  ...
  return normalize({
    ...DEFAULT_APPEARANCE,
    theme: legacyTheme && ALL_THEMES.includes(legacyTheme as typeof ALL_THEMES[number])
      ? legacyTheme
      : DEFAULT_APPEARANCE.theme
  })
}
...
export function watchAppearanceSettings(onChange?: () => void) {
  return watch(
    appearance,
    () => {
      applyAppearanceSettings(appearance.value)
