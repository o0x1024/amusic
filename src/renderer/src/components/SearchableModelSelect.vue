...
function handleEnter() {
  ...
  if (filteredOptions.value[0]) selectOption(filteredOptions.value[0])
}
...
<template>
  <div ref="rootRef" class="relative w-full">
    ...
    >
      ...
      <font-awesome-icon :icon="open ? 'chevron-up' : 'chevron-down'" class="w-3 h-3 shrink-0 opacity-50" />
    </button>

    <div v-if="open" class="absolute z-30 mt-1 w-full rounded-lg border border-base-300 bg-base-100 shadow-lg overflow-hidden">
      ...
      <ul class="menu menu-sm max-h-52 overflow-y-auto p-1">
        <li v-if="allowCustom && query.trim() && !options.includes(query.trim())">
          <button type="button" class="font-mono text-xs text-primary font-bold" @click="selectOption(query.trim())">
            <font-awesome-icon icon="plus" class="w-3 h-3 mr-1" />
            手动输入: {{ query.trim() }}
          </button>
        </li>
        <li v-for="opt in filteredOptions" :key="opt">
          <button type="button" class="font-mono text-xs" :class="{ active: opt === modelValue }" @click="selectOption(opt)">
            {{ opt }}
          </button>
        </li>
